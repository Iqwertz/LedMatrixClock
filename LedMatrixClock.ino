...
const unsigned long TurnOffDelay = 60000; //Delay until the matrix switches off when light level falls under min Brightness
...
{ // Set the Brightness of the Display depending on the ldr readings / if the value drops below a definde value the display is turned off
  lastLDRValues += analogRead(ldrPin);
  LDRsampleNumber ++;
  int ldrStatus=0;
  if(LDRsampleNumber < LightSamples){
    return;
  }else{
    LDRsampleNumber = 0;
    ldrStatus=int(lastLDRValues/LightSamples);
    lastLDRValues=0;
  }
  //sampling
